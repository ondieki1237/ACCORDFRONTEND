...
export function OfflineIndicator() {
  ...
  return (
    <div className="fixed top-4 right-4 z-50 max-w-sm">
      {/* Compact indicator */}
      <Card 
        className={`cursor-pointer transition-all duration-300 rounded-2xl border-0 shadow-xl ${
          status.isOffline 
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            <div className={`rounded-full p-2 ${
              status.isOffline ? 'bg-red-500' : 'bg-amber-500'
            }`}>
              {status.isOffline ? (
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div className="space-y-3 text-sm">
              <div className={`flex items-center gap-3 p-3 rounded-xl ${
                status.isOffline ? 'bg-red-50' : 'bg-green-50'
              }`}>
                {status.isOffline ? (
                ...
                <span className={`font-medium ${
                  status.isOffline ? 'text-red-700' : 'text-green-700'
                }`}>
